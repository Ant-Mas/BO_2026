...
def generate_initial_population(problem: Problem, population_size: int) -> list[list[int]]:
    """
    Generowanie wspólnej populacji początkowej dla obu algorytmów.
    :param problem: Problem do rozwiązania
    :param population_size: Rozmiar populacji
    :return: Lista permutacji (kolejności zdarzeń)
    """
...
def compare_with_shared_population(
        num_vertices: int = 60,
        num_edges: int = 400,
        min_weight: int = 1,
        max_weight: int = 10,
    ...
    """
    Porównanie algorytmu genetycznego i pszczelego z tą samą populacją początkową.
    """
    ...
    print(f"Wygenerowano wspólną populację początkową: {population_size} osobników")
    ...
    print("=" * 50)
    print(f"Shared starting population size: {population_size}")
    print("=" * 50)
    ...
    plt.title('Algorithm comparison (shared starting population)', fontsize=14)
...
def example1():
    compare_with_shared_population(
        seed=78,
        generations=50,
        population_size=70,
        mutation_rate=0.25,
